...
If an opcode returns a result, it is done by pushing it on the stack.
Opcodes marked with ``-`` do not return a result (do not push an item onto the stack),
those marked with ``*`` are special and all others push exactly one item onto the stack.
